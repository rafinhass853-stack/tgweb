...
const ListaVeiculos = () => {
  ...
  // Função para buscar todas as cargas ativas de todos os motoristas
  ...
  // Buscar cargas ativas para cada veículo
  useEffect(() => {
    ...
    const carregarCargas = async () => {
      ...
      try {
        ...
        veiculos.forEach(veiculo => {
          ...
          if (cargaEncontrada) {
            cargasMap[veiculo.id] = cargaEncontrada;
          }
        });
  ...
  const stats = useMemo(() => {
    ...
    // Veículos sem programação com motivos
  ...
  return (
    <div style={{ padding: '40px 20px', backgroundColor: '#000', minHeight: '100vh', fontFamily: 'Inter, sans-serif' }}>
            ...
            return (
              <div key={v.id} style={{
                ...
                <div style={{ padding: '20px' }}>
                  ...
                  {/* Botão para adicionar motivo (se sem programação e sem motivo) */}
                  {!temProgramacao && !motivo && (
                    <button
      ...
      {/* Modal de Motivo de Indisponibilidade */}
      {showMotivoModal && (
        <div style={{
          position: 'fixed',
          top: 0,
